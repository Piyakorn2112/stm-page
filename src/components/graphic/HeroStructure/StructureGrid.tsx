...
import { LIGHT_THEME, type StructureTheme } from "@/utils/structureTheme";
...
// ── Dynamic ring resolution (size-graded) ───────────────────────────────────
...
// level is picked from the cell's DRAWN size and the segment count is GRADED
// smoothly across the field (like a colour grade): the centremost rings carry a
// touch MORE detail, and detail tapers down with the sphere's foreshortening out
// to the tiny rim. This grading drops the broad mid-field below the old flat 160
// while nudging the few centre cells up, so the field is both smoother and ~15%
// cheaper overall vs the previous 4-bucket table. Every combo is an exact 4:1
// segment:piece ratio (⇒ `segments % step == 0`), which is the hard requirement
// that avoids the solid-black-strand bug — see the note in `assemble()`/the memory;
// each new combo here was render-tested clean across 200 seeds. The static
// thumbnail and the live morph for a cell share its level (`resFor(draw)`), so the
// static↔morph handoff stays seamless, and the levels overlap gently in size so
// the change across the field is imperceptible.
type Res = { min: number; seg: number; pc: number };
const RES_LEVELS: Res[] = [
  { min: 90, seg: 168, pc: 42 }, // centremost rings — peak detail (a touch above the old 160)
  { min: 74, seg: 144, pc: 36 },
  { min: 58, seg: 124, pc: 31 },
  { min: 44, seg: 100, pc: 25 },
  { min: 32, seg: 92, pc: 23 },
  { min: 22, seg: 64, pc: 16 },
  { min: 0, seg: 48, pc: 12 }, // the tiniest rim cells — well below the old 64 floor
];
