...
export class ArcWorld {
  ...
  // icon centroids, refreshed before the icon↔icon pass for broad-phase culling
  private icx = new Float64Array(ARC_N);
  private icy = new Float64Array(ARC_N);
  ...
  step(dt: number) {
    ...
    // icon ↔ icon — refresh centroids first, then broad-phase by centroid distance.
    // Two icons can only touch when their centroids are within bodyR+md+bodyR ≤ 64px;
    // a 3·ICON_R reach is conservative (per-frame motion can't cross the margin), so the
    // cull is exact — it only skips pairs that have no overlapping points.
    const { icx, icy } = this;
    ...
    const REACH2 = 9 * ICON_R * ICON_R; // (3·ICON_R)²
    for (let i = 0; i < slots.length; i++) {
      const A = slots[i];
      if (!A.alive) continue;
      const prA = A.pr;
      ...
      for (let j = i + 1; j < slots.length; j++) {
        ...
        const ddx = icx[j] - acx;
        const ddy = icy[j] - acy;
        if (ddx * ddx + ddy * ddy > REACH2) continue; // far apart ⇒ no point can overlap
        const md = prA + B.pr;
        const md2 = md * md;
        for (let a = 0; a < K; a++) {
          for (let b = 0; b < K; b++) {
